...
public class PreprocessDataForMatrixMulRightMapper implements Mapper<LongWritable, Text, IntWritable, DoubleWritable> {
		...
		if(split.length >= 4){	
			try{
				...
				for (int i = 0; i < N; i++){
					...
					out.collect(new IntWritable(i), new DoubleWritable((double)rating * pred1));
				}
			}catch (Exception e) {
				e.printStackTrace();
			}
